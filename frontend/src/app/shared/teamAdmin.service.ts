...
//get all team data 
 get():Observable<Teams>{
  ...
  // get team by id
  getByid(_id:number):Observable<Teams>{
    return this.http.get<Teams>(this.base_url + '/' +_id).pipe(retry(2),catchError(this.handleError));

  }

   // update team by Id the
   update(item :any){
     return this.http.put<Teams>(this.base_url + '/' +item._id,JSON.stringify(item),this.httpOptions).pipe(retry(2),catchError(this.handleError));
   }

    // delete cars
    delete(_id:number){
      return this.http.delete<Teams>(this.base_url + '/' +_id,this.httpOptions).pipe(retry(2),catchError(this.handleError));
