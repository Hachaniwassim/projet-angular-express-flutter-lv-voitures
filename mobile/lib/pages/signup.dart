import 'package:flutter/material.dart';
import 'package:adobe_xd/pinned.dart';
import 'package:flutter_svg/flutter_svg.dart';

class signup extends StatelessWidget {
  final VoidCallback back;
  signup({
    Key? key,
    required this.back,
  }) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xfff4f5fa),
      body: Stack(
        children: <Widget>[
          Pinned.fromPins(
            Pin(start: 18.8, end: 15.4),
            Pin(start: 25.0, end: 27.0),
            child:
                // Adobe XD layer: 'Sign up screen' (group)
                Stack(
              children: <Widget>[
                Pinned.fromPins(
                  Pin(start: 0.0, end: 0.0),
                  Pin(size: 11.4, start: 0.0),
                  child:
                      // Adobe XD layer: 'Status bar' (group)
                      Stack(
                    children: <Widget>[
                      Pinned.fromPins(
                        Pin(size: 46.6, start: 0.0),
                        Pin(size: 8.9, end: 0.0),
                        child:
                            // Adobe XD layer: 'time' (group)
                            Stack(
                          children: <Widget>[
                            Pinned.fromPins(
                              Pin(size: 6.5, start: 0.0),
                              Pin(start: 0.0, end: 0.0),
                              child: SvgPicture.string(
                                _svg_ap8xcu,
                                allowDrawingOutsideViewBox: true,
                                fit: BoxFit.fill,
                              ),
                            ),
                            Pinned.fromPins(
                              Pin(size: 2.0, middle: 0.1844),
                              Pin(start: 1.5, end: 1.0),
                              child: SvgPicture.string(
                                _svg_j9snjz,
                                allowDrawingOutsideViewBox: true,
                                fit: BoxFit.fill,
                              ),
                            ),
                            Pinned.fromPins(
                              Pin(size: 6.7, middle: 0.2985),
                              Pin(start: 0.2, end: 0.2),
                              child: SvgPicture.string(
                                _svg_c5a09g,
                                allowDrawingOutsideViewBox: true,
                                fit: BoxFit.fill,
                              ),
                            ),
                            Pinned.fromPins(
                              Pin(size: 3.7, middle: 0.4636),
                              Pin(start: 0.2, end: 0.2),
                              child: SvgPicture.string(
                                _svg_scgmrz,
                                allowDrawingOutsideViewBox: true,
                                fit: BoxFit.fill,
                              ),
                            ),
                            Pinned.fromPins(
                              Pin(size: 7.7, middle: 0.7384),
                              Pin(start: 0.2, end: 0.2),
                              child: SvgPicture.string(
                                _svg_dh21as,
                                allowDrawingOutsideViewBox: true,
                                fit: BoxFit.fill,
                              ),
                            ),
                            Pinned.fromPins(
                              Pin(size: 8.7, end: 0.0),
                              Pin(start: 0.2, end: 0.2),
                              child: SvgPicture.string(
                                _svg_xhh0,
                                allowDrawingOutsideViewBox: true,
                                fit: BoxFit.fill,
                              ),
                            ),
                          ],
                        ),
                      ),
                      Pinned.fromPins(
                        Pin(size: 64.7, end: 0.0),
                        Pin(start: 0.0, end: 0.4),
                        child:
                            // Adobe XD layer: 'icons' (group)
                            Stack(
                          children: <Widget>[
                            Pinned.fromPins(
                              Pin(size: 14.8, middle: 0.4286),
                              Pin(start: 0.0, end: 0.4),
                              child: Stack(
                                children: <Widget>[
                                  Pinned.fromPins(
                                    Pin(start: 0.0, end: 0.0),
                                    Pin(start: 0.0, end: 0.0),
                                    child: Stack(
                                      children: <Widget>[
                                        Pinned.fromPins(
                                          Pin(start: 0.0, end: 0.0),
                                          Pin(start: 0.0, end: 0.0),
                                          child: SvgPicture.string(
                                            _svg_c7ij9n,
                                            allowDrawingOutsideViewBox: true,
                                            fit: BoxFit.fill,
                                          ),
                                        ),
                                      ],
                                    ),
                                  ),
                                ],
                              ),
                            ),
                            Pinned.fromPins(
                              Pin(size: 16.5, start: 0.0),
                              Pin(start: 0.3, end: 0.3),
                              child: Stack(
                                children: <Widget>[
                                  Pinned.fromPins(
                                    Pin(start: 0.0, end: 0.0),
                                    Pin(start: 0.0, end: 0.0),
                                    child: Stack(
                                      children: <Widget>[
                                        Pinned.fromPins(
                                          Pin(start: 0.0, end: 0.0),
                                          Pin(start: 0.0, end: 0.0),
                                          child: SvgPicture.string(
                                            _svg_ohccmh,
                                            allowDrawingOutsideViewBox: true,
                                            fit: BoxFit.fill,
                                          ),
                                        ),
                                      ],
                                    ),
                                  ),
                                ],
                              ),
                            ),
                            Pinned.fromPins(
                              Pin(size: 23.6, end: 0.0),
                              Pin(start: 0.0, end: 0.0),
                              child: Stack(
                                children: <Widget>[
                                  Pinned.fromPins(
                                    Pin(start: 0.0, end: 0.0),
                                    Pin(start: 0.0, end: 0.0),
                                    child: Stack(
                                      children: <Widget>[
                                        Pinned.fromPins(
                                          Pin(start: 0.0, end: 2.3),
                                          Pin(start: 0.0, end: 0.0),
                                          child: Stack(
                                            children: <Widget>[
                                              Pinned.fromPins(
                                                Pin(start: 0.0, end: 0.0),
                                                Pin(start: 0.0, end: 0.0),
                                                child: SvgPicture.string(
                                                  _svg_k9e38,
                                                  allowDrawingOutsideViewBox:
                                                      true,
                                                  fit: BoxFit.fill,
                                                ),
                                              ),
                                            ],
                                          ),
                                        ),
                                        Pinned.fromPins(
                                          Pin(size: 1.3, end: 0.0),
                                          Pin(size: 3.9, middle: 0.5),
                                          child: SvgPicture.string(
                                            _svg_h4pe5,
                                            allowDrawingOutsideViewBox: true,
                                            fit: BoxFit.fill,
                                          ),
                                        ),
                                        Pinned.fromPins(
                                          Pin(size: 17.5, start: 1.9),
                                          Pin(start: 1.9, end: 1.9),
                                          child: Container(
                                            decoration: BoxDecoration(
                                              borderRadius: BorderRadius.only(
                                                topLeft: Radius.circular(1.29),
                                                topRight: Radius.circular(1.29),
                                                bottomRight:
                                                    Radius.circular(1.29),
                                                bottomLeft:
                                                    Radius.circular(1.29),
                                              ),
                                              color: const Color(0xff4a4b4d),
                                            ),
                                          ),
                                        ),
                                      ],
                                    ),
                                  ),
                                ],
                              ),
                            ),
                          ],
                        ),
                      ),
                    ],
                  ),
                ),
                Pinned.fromPins(
                  Pin(size: 119.0, middle: 0.503),
                  Pin(size: 5.0, end: 0.0),
                  child: Container(
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(10.0),
                      color: const Color(0xff4a4b4d),
                    ),
                  ),
                ),
                Pinned.fromPins(
                  Pin(size: 104.0, middle: 0.5046),
                  Pin(size: 40.0, start: 49.0),
                  child: Text(
                    'Sign Up',
                    style: TextStyle(
                      fontFamily: 'Metropolis',
                      fontSize: 30,
                      color: const Color(0xff4a4b4d),
                      fontWeight: FontWeight.w700,
                    ),
                    textAlign: TextAlign.center,
                  ),
                ),
                Pinned.fromPins(
                  Pin(size: 200.0, middle: 0.5071),
                  Pin(size: 19.0, end: 30.0),
                  child: Text.rich(
                    TextSpan(
                      style: TextStyle(
                        fontFamily: 'Metropolis',
                        fontSize: 14,
                        color: const Color(0xff7c7d7e),
                      ),
                      children: [
                        TextSpan(
                          text: 'Already have an Account? ',
                          style: TextStyle(
                            fontWeight: FontWeight.w500,
                          ),
                        ),
                        TextSpan(
                          text: 'Login',
                          style: TextStyle(
                            color: const Color(0xfffc6011),
                            fontWeight: FontWeight.w700,
                          ),
                        ),
                      ],
                    ),
                    textHeightBehavior:
                        TextHeightBehavior(applyHeightToFirstAscent: false),
                    textAlign: TextAlign.center,
                  ),
                ),
                Container(),
                Container(),
                Container(),
                Container(),
                Container(),
                Container(),
                Container(),
                Pinned.fromPins(
                  Pin(size: 168.0, middle: 0.506),
                  Pin(size: 19.0, start: 101.0),
                  child: Text(
                    'Add your details to sign up',
                    style: TextStyle(
                      fontFamily: 'Metropolis',
                      fontSize: 14,
                      color: const Color(0xff7c7d7e),
                      fontWeight: FontWeight.w500,
                      height: 1.5,
                    ),
                    textHeightBehavior:
                        TextHeightBehavior(applyHeightToFirstAscent: false),
                    textAlign: TextAlign.center,
                  ),
                ),
              ],
            ),
          ),
          buildGroupe16109(context),
        ],
      ),
    );
  }

  Widget buildGroupe16109(context) {
    return Pinned.fromPins(
      Pin(size: 53.0, start: 24.0),
      Pin(size: 52.0, start: 67.0),
      child: GestureDetector(
        onTap: () => back?.call(),
        child: Stack(
          children: <Widget>[
            Pinned.fromPins(
              Pin(start: 0.0, end: 0.0),
              Pin(start: 0.0, end: 0.0),
              child: Container(
                decoration: BoxDecoration(
                  borderRadius:
                      BorderRadius.all(Radius.elliptical(9999.0, 9999.0)),
                  color: const Color(0xe2ba1c1c),
                  boxShadow: [
                    BoxShadow(
                      color: const Color(0x4f0d228b),
                      offset: Offset(0, 5),
                      blurRadius: 20,
                    ),
                  ],
                ),
              ),
            ),
            Pinned.fromPins(
              Pin(size: 17.9, middle: 0.5123),
              Pin(size: 17.4, middle: 0.4859),
              child:
                  // Adobe XD layer: 'arrow-right-solid' (shape)
                  SvgPicture.string(
                _svg_kebtc,
                allowDrawingOutsideViewBox: true,
                fit: BoxFit.fill,
              ),
            ),
          ],
        ),
      ),
    );
  }
}

const String _svg_ap8xcu =
    '<svg viewBox="0.0 0.0 6.5 8.9" ><path transform="translate(-26.02, -18.33)" d="M 29.11100006103516 27.20499992370605 C 28.33699989318848 27.20499992370605 27.67900085449219 26.9950008392334 27.13599967956543 26.57500076293945 C 26.5930004119873 26.15500068664551 26.25900077819824 25.59799957275391 26.13400077819824 24.90200042724609 L 27.63999938964844 24.90200042724609 C 27.73800086975098 25.22299957275391 27.91799926757812 25.47599983215332 28.18199920654297 25.6609992980957 C 28.44599914550781 25.84700012207031 28.75900077819824 25.93899917602539 29.12299919128418 25.93899917602539 C 29.73999977111816 25.93899917602539 30.22200012207031 25.66699981689453 30.56999969482422 25.12199974060059 C 30.91699981689453 24.57699966430664 31.09099960327148 23.81999969482422 31.09099960327148 22.85199928283691 L 31.03899955749512 22.85199928283691 L 31.00900077819824 22.85199928283691 L 30.99200057983398 22.85199928283691 C 30.79199981689453 23.25399971008301 30.50099945068359 23.56500053405762 30.11899948120117 23.7859992980957 C 29.73600006103516 24.00699996948242 29.29599952697754 24.11700057983398 28.79999923706055 24.11700057983398 C 27.99600028991699 24.11700057983398 27.33099937438965 23.85199928283691 26.80500030517578 23.31999969482422 C 26.28000068664551 22.78899955749512 26.01700019836426 22.11700057983398 26.01700019836426 21.30500030517578 C 26.01700019836426 20.4379997253418 26.31100082397461 19.72599983215332 26.89900016784668 19.16900062561035 C 27.48699951171875 18.61199951171875 28.23999977111816 18.33399963378906 29.15800094604492 18.33399963378906 C 29.77499961853027 18.33399963378906 30.32200050354004 18.47699928283691 30.79800033569336 18.76199913024902 C 31.27499961853027 19.04700088500977 31.6560001373291 19.46100044250488 31.94099998474121 20.00399971008301 C 32.3390007019043 20.71100044250488 32.53900146484375 21.60199928283691 32.53900146484375 22.67600059509277 C 32.53900146484375 24.09799957275391 32.23600006103516 25.20800018310547 31.6299991607666 26.00699996948242 C 31.02499961853027 26.80599975585938 30.18499946594238 27.20499992370605 29.11100006103516 27.20499992370605 Z M 27.9680004119873 22.45000076293945 C 28.2810001373291 22.76099967956543 28.67900085449219 22.91600036621094 29.16399955749512 22.91600036621094 C 29.64800071716309 22.91600036621094 30.04800033569336 22.76199913024902 30.36499977111816 22.45299911499023 C 30.68099975585938 22.14500045776367 30.8390007019043 21.75399971008301 30.8390007019043 21.2810001373291 C 30.8390007019043 20.80500030517578 30.67900085449219 20.40399932861328 30.35899925231934 20.07999992370605 C 30.03899955749512 19.75600051879883 29.64200019836426 19.5939998626709 29.16900062561035 19.5939998626709 C 28.69700050354004 19.5939998626709 28.29999923706055 19.75300025939941 27.97999954223633 20.07099914550781 C 27.65999984741211 20.38999938964844 27.5 20.78499984741211 27.5 21.25799942016602 C 27.5 21.74200057983398 27.6560001373291 22.13999938964844 27.9680004119873 22.45000076293945 Z" fill="#4a4b4d" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_j9snjz =
    '<svg viewBox="8.2 1.5 2.0 6.4" ><path transform="translate(-26.02, -18.33)" d="M 34.25500106811523 20.77700042724609 C 34.25500106811523 20.48800086975098 34.34500122070312 20.25399971008301 34.52500152587891 20.07399940490723 C 34.70500183105469 19.89500045776367 34.94300079345703 19.80500030517578 35.2400016784668 19.80500030517578 C 35.5369987487793 19.80500030517578 35.77399826049805 19.89500045776367 35.95199966430664 20.07399940490723 C 36.12900161743164 20.25399971008301 36.21799850463867 20.48800086975098 36.21799850463867 20.77700042724609 C 36.21799850463867 21.0629997253418 36.12900161743164 21.29500007629395 35.95199966430664 21.47500038146973 C 35.77399826049805 21.65399932861328 35.5369987487793 21.74399948120117 35.2400016784668 21.74399948120117 C 34.94300079345703 21.74399948120117 34.70500183105469 21.65399932861328 34.52500152587891 21.47500038146973 C 34.34500122070312 21.29500007629395 34.25500106811523 21.0629997253418 34.25500106811523 20.77700042724609 Z M 34.25500106811523 25.19499969482422 C 34.25500106811523 24.90999984741211 34.34500122070312 24.67799949645996 34.52500152587891 24.49799919128418 C 34.70500183105469 24.31800079345703 34.94300079345703 24.22900009155273 35.2400016784668 24.22900009155273 C 35.5369987487793 24.22900009155273 35.77399826049805 24.31800079345703 35.95199966430664 24.49799919128418 C 36.12900161743164 24.67799949645996 36.21799850463867 24.90999984741211 36.21799850463867 25.19499969482422 C 36.21799850463867 25.48399925231934 36.12900161743164 25.7189998626709 35.95199966430664 25.89800071716309 C 35.77399826049805 26.07799911499023 35.5369987487793 26.16799926757812 35.2400016784668 26.16799926757812 C 34.94300079345703 26.16799926757812 34.70500183105469 26.07799911499023 34.52500152587891 25.89800071716309 C 34.34500122070312 25.7189998626709 34.25500106811523 25.48399925231934 34.25500106811523 25.19499969482422 Z" fill="#4a4b4d" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_c5a09g =
    '<svg viewBox="11.9 0.2 6.7 8.5" ><path transform="translate(-26.02, -18.33)" d="M 42.06000137329102 27 L 42.06000137329102 25.42399978637695 L 37.92900085449219 25.42399978637695 L 37.92900085449219 24.07600021362305 C 38.61700057983398 22.81100082397461 39.76499938964844 20.96699905395508 41.375 18.54500007629395 L 43.50699996948242 18.54500007629395 L 43.50699996948242 24.14599990844727 L 44.65000152587891 24.14599990844727 L 44.65000152587891 25.42399978637695 L 43.50699996948242 25.42399978637695 L 43.50699996948242 27 L 42.06000137329102 27 Z M 39.33000183105469 24.0939998626709 L 39.33000183105469 24.18199920654297 L 42.08300018310547 24.18199920654297 L 42.08300018310547 19.77499961853027 L 42.00099945068359 19.77499961853027 C 41.08700180053711 21.12700080871582 40.19699859619141 22.56599998474121 39.33000183105469 24.0939998626709 Z" fill="#4a4b4d" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_scgmrz =
    '<svg viewBox="19.9 0.2 3.7 8.5" ><path transform="translate(-26.02, -18.33)" d="M 48.11299896240234 27 L 48.11299896240234 20.06800079345703 L 48.01300048828125 20.06800079345703 L 45.90999984741211 21.54500007629395 L 45.90999984741211 20.09199905395508 L 48.11899948120117 18.54500007629395 L 49.625 18.54500007629395 L 49.625 27 L 48.11299896240234 27 Z" fill="#4a4b4d" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_dh21as =
    '<svg viewBox="28.7 0.2 7.7 8.5" ><path transform="translate(-26.02, -18.33)" d="M 60.8390007019043 27 L 60.125 24.83799934387207 L 57.01300048828125 24.83799934387207 L 56.2869987487793 27 L 54.75699996948242 27 L 57.75699996948242 18.54500007629395 L 59.45700073242188 18.54500007629395 L 62.46200180053711 27 L 60.8390007019043 27 Z M 58.52500152587891 20.1560001373291 L 57.3650016784668 23.64299964904785 L 59.77299880981445 23.64299964904785 L 58.625 20.1560001373291 L 58.52500152587891 20.1560001373291 Z" fill="#4a4b4d" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_xhh0 =
    '<svg viewBox="37.9 0.2 8.7 8.5" ><path transform="translate(-26.02, -18.33)" d="M 65.32199859619141 27 L 63.91600036621094 27 L 63.91600036621094 18.54500007629395 L 65.68499755859375 18.54500007629395 L 68.22799682617188 24.70299911499023 L 68.33300018310547 24.70299911499023 L 70.87599945068359 18.54500007629395 L 72.64600372314453 18.54500007629395 L 72.64600372314453 27 L 71.23999786376953 27 L 71.23999786376953 21.05299949645996 L 71.15799713134766 21.05299949645996 L 68.81999969482422 26.67200088500977 L 67.74199676513672 26.67200088500977 L 65.40399932861328 21.05299949645996 L 65.32199859619141 21.05299949645996 L 65.32199859619141 27 Z" fill="#4a4b4d" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_c7ij9n =
    '<svg viewBox="0.0 0.0 14.8 10.6" ><path transform="translate(-315.69, -15.83)" d="M 325.2529907226562 24.42799949645996 C 325.31298828125 24.36800003051758 325.3450012207031 24.28800010681152 325.343994140625 24.20299911499023 C 325.3420104980469 24.1200008392334 325.3049926757812 24.04000091552734 325.2439880371094 23.98399925231934 C 324.0060119628906 22.9370002746582 322.1940002441406 22.9370002746582 320.9559936523438 23.98399925231934 C 320.8930053710938 24.04000091552734 320.8580017089844 24.1200008392334 320.8559875488281 24.20299911499023 C 320.85400390625 24.28800010681152 320.885986328125 24.36800003051758 320.9460144042969 24.42799949645996 L 322.885009765625 26.38400077819824 C 322.9410095214844 26.44099998474121 323.0190124511719 26.47299957275391 323.1000061035156 26.47299957275391 C 323.1809997558594 26.47299957275391 323.2579956054688 26.44099998474121 323.3139953613281 26.38400077819824 L 325.2529907226562 24.42799949645996 Z M 323.1000061035156 21.72500038146973 C 324.2820129394531 21.72500038146973 325.4200134277344 22.16500091552734 326.2969970703125 22.95700073242188 C 326.4140014648438 23.06999969482422 326.6010131835938 23.06800079345703 326.7160034179688 22.95299911499023 L 327.8370056152344 21.81999969482422 C 327.89599609375 21.76099967956543 327.9280090332031 21.68000030517578 327.9280090332031 21.59600067138672 C 327.9259948730469 21.51300048828125 327.8930053710938 21.4330005645752 327.8320007324219 21.375 C 325.1650085449219 18.89399909973145 321.0360107421875 18.89399909973145 318.3689880371094 21.375 C 318.3089904785156 21.4330005645752 318.2739868164062 21.51300048828125 318.2739868164062 21.59600067138672 C 318.2730102539062 21.68000030517578 318.3059997558594 21.76099967956543 318.364990234375 21.81999969482422 L 319.4849853515625 22.95299911499023 C 319.6010131835938 23.06800079345703 319.7869873046875 23.06999969482422 319.906005859375 22.95700073242188 C 320.781005859375 22.16500091552734 321.9190063476562 21.72599983215332 323.1000061035156 21.72500038146973 Z M 323.1000061035156 18.04299926757812 C 325.2510070800781 18.04299926757812 327.3179931640625 18.86899948120117 328.8760070800781 20.35000038146973 C 328.9939880371094 20.46500015258789 329.1809997558594 20.46299934387207 329.2969970703125 20.34700012207031 L 330.4179992675781 19.21500015258789 C 330.4779968261719 19.1560001373291 330.5090026855469 19.07699966430664 330.5090026855469 18.99300003051758 C 330.5090026855469 18.90999984741211 330.4760131835938 18.82999992370605 330.4150085449219 18.77300071716309 C 326.3250122070312 14.85299968719482 319.8729858398438 14.85299968719482 315.7829895019531 18.77300071716309 C 315.7229919433594 18.82999992370605 315.6900024414062 18.90999984741211 315.6889953613281 18.99300003051758 C 315.6889953613281 19.07600021362305 315.7210083007812 19.1560001373291 315.7799987792969 19.21500015258789 L 316.9030151367188 20.34700012207031 C 317.0180053710938 20.46299934387207 317.2059936523438 20.46500015258789 317.322998046875 20.35000038146973 C 318.8810119628906 18.86800003051758 320.9490051269531 18.04299926757812 323.1000061035156 18.04299926757812 Z" fill="#4a4b4d" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_ohccmh =
    '<svg viewBox="0.0 0.0 16.5 10.3" ><path transform="translate(-294.32, -16.16)" d="M 308.8699951171875 16.15900039672852 L 309.8399963378906 16.15900039672852 C 310.3770141601562 16.15900039672852 310.8110046386719 16.5930004119873 310.8110046386719 17.12899971008301 L 310.8110046386719 25.53800010681152 C 310.8110046386719 26.07500076293945 310.3770141601562 26.50799942016602 309.8399963378906 26.50799942016602 L 308.8699951171875 26.50799942016602 C 308.3349914550781 26.50799942016602 307.8999938964844 26.07500076293945 307.8999938964844 25.53800010681152 L 307.8999938964844 17.12899971008301 C 307.8999938964844 16.5930004119873 308.3349914550781 16.15900039672852 308.8699951171875 16.15900039672852 Z M 304.3429870605469 18.42300033569336 L 305.31298828125 18.42300033569336 C 305.8489990234375 18.42300033569336 306.2839965820312 18.85700035095215 306.2839965820312 19.39299964904785 L 306.2839965820312 25.53800010681152 C 306.2839965820312 26.07500076293945 305.8489990234375 26.50799942016602 305.31298828125 26.50799942016602 L 304.3429870605469 26.50799942016602 C 303.8059997558594 26.50799942016602 303.3729858398438 26.07500076293945 303.3729858398438 25.53800010681152 L 303.3729858398438 19.39299964904785 C 303.3729858398438 18.85700035095215 303.8059997558594 18.42300033569336 304.3429870605469 18.42300033569336 Z M 299.8150024414062 20.6870002746582 L 300.7860107421875 20.6870002746582 C 301.3210144042969 20.6870002746582 301.7560119628906 21.12199974060059 301.7560119628906 21.6569995880127 L 301.7560119628906 25.53800010681152 C 301.7560119628906 26.07500076293945 301.3210144042969 26.50799942016602 300.7860107421875 26.50799942016602 L 299.8150024414062 26.50799942016602 C 299.2789916992188 26.50799942016602 298.8450012207031 26.07500076293945 298.8450012207031 25.53800010681152 L 298.8450012207031 21.6569995880127 C 298.8450012207031 21.12199974060059 299.2789916992188 20.6870002746582 299.8150024414062 20.6870002746582 Z M 295.2860107421875 22.62800025939941 L 296.2560119628906 22.62800025939941 C 296.7919921875 22.62800025939941 297.2260131835938 23.0629997253418 297.2260131835938 23.59799957275391 L 297.2260131835938 25.53800010681152 C 297.2260131835938 26.07500076293945 296.7919921875 26.50799942016602 296.2560119628906 26.50799942016602 L 295.2860107421875 26.50799942016602 C 294.75 26.50799942016602 294.3150024414062 26.07500076293945 294.3150024414062 25.53800010681152 L 294.3150024414062 23.59799957275391 C 294.3150024414062 23.0629997253418 294.75 22.62800025939941 295.2860107421875 22.62800025939941 Z" fill="#4a4b4d" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_k9e38 =
    '<svg viewBox="0.0 0.0 21.3 11.0" ><path transform="translate(-335.39, -15.84)" d="M 354.1539916992188 16.80599975585938 C 355.0450134277344 16.80599975585938 355.7699890136719 17.5310001373291 355.7699890136719 18.42300033569336 L 355.7699890136719 24.2450008392334 C 355.7699890136719 25.13699913024902 355.0450134277344 25.86199951171875 354.1539916992188 25.86199951171875 L 337.9800109863281 25.86199951171875 C 337.0889892578125 25.86199951171875 336.364013671875 25.13699913024902 336.364013671875 24.2450008392334 L 336.364013671875 18.42300033569336 C 336.364013671875 17.5310001373291 337.0889892578125 16.80599975585938 337.9800109863281 16.80599975585938 L 354.1539916992188 16.80599975585938 M 354.1539916992188 15.83500003814697 L 337.9800109863281 15.83500003814697 C 336.5509948730469 15.83500003814697 335.3940124511719 16.99399948120117 335.3940124511719 18.42300033569336 L 335.3940124511719 24.2450008392334 C 335.3940124511719 25.67399978637695 336.5509948730469 26.83300018310547 337.9800109863281 26.83300018310547 L 354.1539916992188 26.83300018310547 C 355.5830078125 26.83300018310547 356.739990234375 25.67399978637695 356.739990234375 24.2450008392334 L 356.739990234375 18.42300033569336 C 356.739990234375 16.99399948120117 355.5830078125 15.83500003814697 354.1539916992188 15.83500003814697 L 354.1539916992188 15.83500003814697 Z" fill="#4a4b4d" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_h4pe5 =
    '<svg viewBox="22.3 3.6 1.3 3.9" ><path transform="translate(-335.39, -15.84)" d="M 357.7109985351562 19.39299964904785 L 357.7109985351562 23.27499961853027 C 358.4920043945312 22.94599914550781 359 22.18099975585938 359 21.33399963378906 C 359 20.48699951171875 358.4920043945312 19.72200012207031 357.7109985351562 19.39299964904785" fill="#4a4b4d" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_kebtc =
    '<svg viewBox="18.0 16.8 17.9 17.4" ><defs><filter id="shadow"><feDropShadow dx="0" dy="3" stdDeviation="6"/></filter></defs><path transform="translate(18.0, -20.85)" d="M 10.27357959747314 38.81678009033203 L 9.388029098510742 37.93121337890625 C 9.013066291809082 37.55625915527344 8.406743049621582 37.55625915527344 8.035769462585449 37.93121337890625 L 0.2812193632125854 45.68180465698242 C -0.09374169260263443 46.0567626953125 -0.09374169260263443 46.66308212280273 0.2812193632125854 47.0340576171875 L 8.035769462585449 54.78861618041992 C 8.410731315612793 55.1635627746582 9.017055511474609 55.1635627746582 9.388029098510742 54.78861618041992 L 10.27357959747314 53.90307235717773 C 10.65253067016602 53.52411270141602 10.64455318450928 52.90582656860352 10.25762462615967 52.53485488891602 L 5.450918674468994 47.95550537109375 L 16.91520881652832 47.95550537109375 C 17.44574165344238 47.95550537109375 17.87256050109863 47.52869033813477 17.87256050109863 46.99814224243164 L 17.87256050109863 45.72169876098633 C 17.87256050109863 45.19115829467773 17.44574165344238 44.76433563232422 16.91520881652832 44.76433563232422 L 5.450918674468994 44.76433563232422 L 10.25762462615967 40.18498611450195 C 10.64854335784912 39.81402206420898 10.65652179718018 39.19571685791016 10.27357959747314 38.81678009033203 Z" fill="#0b0a0a" fill-opacity="0.91" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" filter="url(#shadow)"/></svg>';
